...
myobject.x = 200;
console.log('Data: ' + myobject.data); // Calling get data.
console.log('x: ' + myobject.x);
myobject.data = 100; // set data.
console.log('Data:' + myobject.data);
// a comma(13th line) is required after the function body (get) to separate the 
...
class RegisterNumber {
	#number; // Private field is legal inside java class but illegal in java script object.
	constructor() {
		this.#number = 1;
	}
	...
	callPrint() {
		const x = this.#printToScreen(); // Call private function within same class.
		return x;
	}
}
